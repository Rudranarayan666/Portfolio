    ...
    <section id="about" className="py-12 sm:py-16 relative scroll-mt-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6 lg:gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 neo-box p-5 sm:p-8 bg-white flex flex-col justify-between space-y-5 sm:space-y-6">
            <div className="space-y-3 sm:space-y-4">
              ...
              <p className="text-zinc-800 text-sm sm:text-base lg:text-lg leading-relaxed font-medium">
            ...
            {/* 3 Simple Pillars */}
            <div className="pt-3 sm:pt-4 border-t-2 border-black grid grid-cols-3 gap-2 text-center">
              <div className="p-2 rounded-lg bg-zinc-100 border-2 border-black">
                ...
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-600">React & APIs</div>
              </div>
              <div className="p-2 rounded-lg bg-zinc-100 border-2 border-black">
                <div className="font-heading font-bold text-xs sm:text-sm text-black">AI & ML</div>
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-600">Ensembles & XAI</div>
              </div>
              <div className="p-2 rounded-lg bg-zinc-100 border-2 border-black">
                <div className="font-heading font-bold text-xs sm:text-sm text-black">Community</div>
                <div className="text-[10px] sm:text-[11px] font-mono text-zinc-600">100+ Guided</div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between gap-3 sm:gap-4">
            {cards.map((card, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="neo-box neo-box-hover p-4 sm:p-5 bg-white flex items-start space-x-3.5 sm:space-x-4"
                >
                  <div className={`p-2.5 sm:p-3 rounded-xl border-2 border-black shadow-[2px_2px_0px_0px_#000] ${card.bg} shrink-0`}>
                    <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-black" />
                  </div>
                  <div className="space-y-0.5">
                    <span className="font-mono text-[10px] sm:text-[11px] font-extrabold text-zinc-600 tracking-wider">
                      {card.badge}
                    </span>
                    <h3 className="font-heading font-bold text-sm sm:text-base text-black">
                      {card.title}
                    </h3>
                    <p className="text-xs text-zinc-700 font-medium">
                      {card.subtitle}
                    </p>
                    <p className="text-[10px] sm:text-[11px] font-mono font-bold text-zinc-500 pt-0.5">
                      {card.meta}
                    </p>
                  </div>
                </div>
